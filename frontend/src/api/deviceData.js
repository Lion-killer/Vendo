...
// Імпорт лише чистого storageKeys → тестується через `node --test`.
import { K, UI_PREFS } from '../storageKeys.js';

export const DEVICE_PRESERVE = UI_PREFS;
...
export const purgeOnDeviceSwitch = (newDeviceId, storage) => {
    ...
    const prev = storage.getItem(K.deviceId);
