...
// До моделі доклеюється екран "ширина×висота @DPR" (CSS-пікселі вьюпорта; фізичні =
// CSS × DPR) — для розбору проблем верстки на конкретних екранах. Окреме поле в
// регістрі телеметрії не потрібне: оператор бачить це в колонці «Модель».
const deviceInfo = () => {
    const ua = navigator.userAgent;
    const m = /;\s*([^;)]+)\s+Build\//.exec(ua);
    const os = /Android\s([\d.]+)/.exec(ua);
    // Фолбек на вьюпорт: у вбудованих webview screen.* буває 0×0.
    const scr = `${window.screen.width || window.innerWidth}×${window.screen.height || window.innerHeight}`
