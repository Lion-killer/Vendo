# 9. Налаштування

> **Коли це потрібно:** змінити вигляд чи мову, очистити дані або вийти.

Усе — у **меню профілю**: натисни **аватар** (кружок з ініціалами) угорі ліворуч на головному екрані. `[скріншот: меню профілю]`

## Тема
Перемикач **світла / темна**. Зберігається на пристрої.

## Мова
**Українська / російська / англійська**. Інтерфейс і повідомлення перемикаються одразу.

## Очистити дані
Прибирає кеш, чернетки та локальні налаштування списків. **Авторизація зберігається** (повторно входити не треба), дані заново завантажаться з офісу.
- Якщо є **невідправлені замовлення**, додаток **попередить** — спершу синхронізуйся, інакше їх буде втрачено. `[скріншот: попередження про очищення з невідправленими]`

## Вийти
Завершує сеанс і повертає на екран входу. Щоб зайти знову — потрібен **новий QR** від адміністратора.
